...
def clock_in(uname, passwd, name, Location):
    ...
    while i < 3:
        try:
            ...
            login_btn = bro.find_element_by_xpath(
                '/html/body/div[2]/div[1]/div[2]/div/div[1]/div/div/form[1]/div[6]/input[5]')
            login_btn.click()
            sleep(2)
        except:
            print('第' + str(i) + '次登录门户网站失败')  # （tips:三次登陆失败会出验证码）
            i = i + 1
            continue
        # 进行是否登陆成功的检查
        try:
            ...
            out = bro.find_element_by_xpath(
                '/html/body/div/div[1]/div[1]/div/div[1]/div[2]/div/div/div/div/div/div/div/li[6]/span').get_attribute("textContent")
        except:
            print('第' + str(i) + '次登录门户网站验证失败')
        else:
            if "退出" in out:
                print('信息门户登录完成')
                break
    ...
    while j < 3:
        ...
        try:
            ...
            old_wz = wz.get_attribute("textContent")
            wz.send_keys(Keys.CONTROL,"a")
            wz.send_keys(Keys.DELETE)
    ...
    try:
        ...
        bro.get_screenshot_as_file(u"/root/screenshot/" + "3" + name + time_format() + ".png")
    else:
